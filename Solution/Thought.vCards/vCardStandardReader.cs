    ...
    public class vCardStandardReader : vCardReader
    {
        ...
        public void ReadInto(vCard card, vCardProperty property)
        {
            ...
            switch (property.Name.ToUpperInvariant())
            {
                ...
                case "VERSION":
                    ReadInto_VERSION(card, property);
                    break;
        ...
        #region [ ReadInto_VERSION ]

        /// <summary>
        ///     Reads the VERSION property.
        /// </summary>
        private void ReadInto_VERSION(vCard card, vCardProperty property)
        {

            if (property.Value == null)
                return;

            char version = property.Value.ToString()[0];

            switch (version)
            {
                case '2':
                    card.Version = vCardVersion.Version2;
                    break;
                ...
                case '4':
                    card.Version = vCardVersion.Version4;
                    break;
            }
        ...
        /// </summary>
        private void ReadInto_GENDER(vCard card, vCardProperty property)
        {
        ...
        public vCardProperty ReadProperty(TextReader reader)
        {
            ...
            do
            {
                ...
                for (int index = 1; index < nameParts.Length; index++)
                {
                    ...
                    if (subNameValue.Length == 2 && string.Compare(subNameValue[0], "TYPE", StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        subNameValue = new[] { subNameValue[1] };
                    }
